  ...
  const handleRefreshMetrics = async () => {
    ...
    try {
      if (refreshData) {
        ...
        // Fallback timeout if context doesn't provide refresh function
      ...
      <motion.div variants={fadeUpItem} className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            disabled={refreshing}
            className="flex items-center gap-1.5 h-8 px-3 rounded-lg text-xs font-black transition-all cursor-pointer border hover:opacity-80 active:scale-95 disabled:opacity-50 shadow-sm"
            style={{ background: '#faf6f0', borderColor: 'rgba(200,131,74,0.2)', color: '#c8834a' }}
          >
