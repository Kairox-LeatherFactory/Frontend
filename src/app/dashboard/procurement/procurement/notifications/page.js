...
export default function Notifications() {
  ...
  const load = async () => setRows((await apiGetNotifications(token)).notifications);
  ...
  const open = async (id) => {
    await apiOpenNotification(token, id);
    await load();
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5 pb-12">
      ...
      {!rows ? (
        ...
        rows.map((n) => (
          <SpotlightCard key={n.id} className="p-4 rounded-2xl bg-white" spotlightColor="rgba(200,131,74,.04)" style={{ border: '1px solid rgba(200,131,74,.15)' }}>
            <div className="flex gap-3">
              <Bell className="w-5 h-5 text-[#c8834a] mt-1" />
              <div className="flex-1">
                <div className="flex justify-between gap-3">
                  <b className="text-sm">{n.title}</b>
                  {n.read && <Check className="w-4 h-4 text-green-600" />}
                </div>
                <p className="text-xs text-slate-500 mt-1">{n.message}</p>
                {!n.read && (
                  <button onClick={() => open(n.id)} className="mt-3 px-3 py-1.5 rounded-lg bg-slate-100 text-[10px] font-black">
                    Open
                  </button>
                )}
              </div>
