...
export default function BOMReviewPage() {
      ...
      setTimeout(() => {
        ...
        if (!localStorage.getItem(alertKey)) {
          ...
          fetch('/api/send-inventory-alert', {
            ...
            body: JSON.stringify({
              subject: '[PTE URGENT] Stock Shortage — Please Confirm Availability',
              text: shortageText,
            })
          }).then(async res => {
            if (res.ok) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      doc.text(`Generated by PTE Factory Intelligence Platform  •  ${new Date().toLocaleString()}`, pageW / 2, 287, { align: 'center' });
