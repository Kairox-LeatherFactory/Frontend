...
} from '@/store/slices/progressapiSlice';
...
export default function StyleStageProgress() {
  ...
  const [triggerGetOrders, { isFetching: ordersLoading }] = useLazyGetBarcodeOrdersQuery();
  const [triggerGetOrderDetail, { isFetching: orderDetailLoading }] = useLazyGetDirectManagerOrderDetailQuery();
  const [triggerGetOrderStyleTree, { isFetching: orderStyleTreeLoading }] = useLazyGetOrderTreeQuery();
  const [triggerGetOrderSkus, { isFetching: stylesLoading }] = useLazyGetOrderBarcodeSkusQuery();
  const [triggerGetStyleDetail, { isFetching: styleDetailLoading }] = useLazyGetDirectManagerStyleDetailQuery();
  const [triggerGetPieces, { isFetching: piecesLoading }] = useLazyGetOrderBarcodesQuery();
  const [triggerGetPieceDetail, { isFetching: pieceDetailLoading }] = useLazyGetDirectManagerPieceDetailQuery();

  useEffect(() => {
    triggerGetOrders().unwrap()
  ...
  useEffect(() => {
    if (!selectedOrderId) { setOrderDetail(null); setStyleOptions([]); setOrderStyleTree(null); return; }
    setSelectedStyleId('');
    ...
    setPieceOptions([]);
      ...
      triggerGetOrderDetail(selectedOrderId).unwrap()
      .then((data) => setOrderDetail(data))
      .catch(() => setOrderDetail(null));

    triggerGetOrderStyleTree(selectedOrderId).unwrap()
      .then((data) => setOrderStyleTree(data))
      .catch(() => setOrderStyleTree(null));
    triggerGetOrderSkus(selectedOrderId).unwrap()
      .then((skus) => {
  ...
  useEffect(() => {
    if (!selectedStyleId) { setStyleDetail(null); setPieceOptions([]); return; }
    setSelectedPieceCode('');
    setPieceDetail(null);
    triggerGetStyleDetail(selectedStyleId).unwrap()
  ...
  }, [selectedStyleId, selectedOrderId]);
  // Piece selected -> full piece stage history
   useEffect(() => {
    if (!selectedPieceCode) { setPieceDetail(null); return; }
    triggerGetPieceDetail(selectedPieceCode).unwrap()
