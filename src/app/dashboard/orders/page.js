...
import { Building2, Plus, X, Loader2, PackagePlus, Calendar, Ship, CheckCircle2, XCircle, Search } from 'lucide-react';
import SpotlightCard from '@/components/SpotlightCard';
import { createPortal } from 'react-dom';

export default function OrdersTreeBrowser() {
  const { clients = [], createClient, addClientOrder, apiLoading } = useData();
  const { user } = useAuth();
  ...
  const [newCountry, setNewCountry] = useState(''); // 👈 Country State added
  ...
  // Filter Logic: Search by Client Name or Company Code
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-4 pt-3 flex items-center justify-between text-xs" style={{ borderTop: '1px solid rgba(200,131,74,0.1)' }}>
                      <span className="text-[10px] font-bold uppercase" style={{ color: '#9a7a5a' }}>Actions</span>
          ...
          >
            ...
            <form
              onSubmit={async (e) => {
                ...
                try {
                  if (createClient) {
                    // Pass country along if your createClient function accepts it
                    await createClient(newClientName.trim(), newCompanyCode.trim(), newOrderNumber.trim(), newCountry.trim());
                  } else {
                    const newClient = {
