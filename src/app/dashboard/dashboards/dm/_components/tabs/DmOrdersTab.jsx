...
/**
 * ============================================================================
 * DmOrdersTab Component (Direct Manager)
 * ============================================================================
 */
export default function DmOrdersTab({
  orderProgress = [],
}) {
  return (
    <motion.div
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h3 className="text-base font-black text-slate-900 flex items-center gap-2">
              <Waypoints className="w-5 h-5 text-slate-800" />
              Executive Purchase Order Matrix
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Plant fulfillment status across active purchase orders
            </p>
          </div>
          ...
          <table className="w-full text-xs md:text-sm text-left">
            <thead>
              <tr className="bg-slate-50 text-slate-600 font-bold uppercase tracking-wider border-b border-slate-200">
                ...
                <th className="py-3 px-4 text-right">Pending Qty</th>
            ...
            <tbody className="divide-y divide-slate-100 font-medium">
              {orderProgress.map((row, idx) => {
                ...
                return (
                  <tr key={`dm-order-${row.order_id || row.order_number}-${idx}`} className="hover:bg-slate-50 transition-colors">
