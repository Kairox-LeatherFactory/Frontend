...
// (order numbers, cutter names) that popup can render past the viewport edge
// on a tablet. This is a fully custom dropdown instead: the open panel is
// pinned left:0/right:0 against its own button, so its width is always
  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between gap-2 bg-[#f8fafc] border border-slate-200 rounded-xl px-2.5 py-1.5 text-xs font-bold text-slate-700 focus:outline-none focus:border-[#2563eb] cursor-pointer"
      >
        ...
        <div className="absolute left-0 right-0 z-50 mt-1 max-h-64 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-lg py-1">
          {placeholder && (
            <button
              ...
              className={`w-full text-left px-3 py-1.5 text-xs font-bold truncate cursor-pointer hover:bg-slate-50 ${value === 'all' ? 'text-[#2563eb] bg-blue-50' : 'text-slate-700'}`}
            >
              {placeholder}
            </button>
          )}
          {options.map((opt, idx) => (
            <button
              ...
              className={`w-full text-left px-3 py-1.5 text-xs font-bold truncate cursor-pointer hover:bg-slate-50 ${value === opt.value ? 'text-[#2563eb] bg-blue-50' : 'text-slate-700'}`}
            >
              {opt.label}
            </button>
          ))}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-8 gap-2">
          ...
          <div>
            <ScreenSafeSelect
              ...
              options={ordersList.map((ord) => ({
                value: ord.id,
                label: `${ord.client ? `${ord.client} - ` : ''}PO: ${ord.order_number}`,
              }))}
            />
          </div>
          ...
          <div>
            <ScreenSafeSelect
              value={filterCutter}
              onChange={(v) => { setFilterCutter(v); setCurrentPage(1); }}
              placeholder="✂️ All Cutters"
              options={availableCutters.map((c) => ({ value: c.id, label: c.name }))}
            />
          </div>
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div className="min-w-0">
                        <h4 className="text-sm font-extrabold text-slate-900 truncate">{cutter.name}</h4>
                        <p className="text-xs text-slate-500 truncate">{cutter.designation || '—'}</p>
                      </div>
                    </div>
            ...
            >
              <div className="flex items-center justify-between gap-3 pb-3 border-b border-slate-100">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#2563eb] to-[#1d4ed8] flex items-center justify-center text-white font-black text-xs shrink-0">
                    {initials(selectedCutterModal.name)}
                  </div>
                  <div className="min-w-0">
                    <h3 className="text-sm font-extrabold text-slate-900 truncate">{selectedCutterModal.name}</h3>
                    <p className="text-xs text-slate-500 truncate">{selectedCutterModal.designation || '—'} &bull; Assigned Pieces: {selectedCutterModal.assigned_pieces ?? 0}</p>
                  </div>
                </div>
                <button onClick={() => setSelectedCutterModal(null)} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-200 shrink-0">
                  <X className="w-4 h-4" />
                </button>
              </div>
